  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createSupportTicket({
        subject: `Support Request from ${formData.name} - ${formData.labName}`,
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        console.log('📋 Fetching laboratory support tickets...')
        const response = await getSupportTickets()
        console.log('📋 Laboratory support tickets response:', response)
        
        if (response.success && response.data) {
          // Backend returns { items: [...], pagination: {...} }
          const ticketsData = response.data.items || response.data || []
          console.log('✅ Processed laboratory tickets:', ticketsData)
          setTickets(ticketsData)
        } else {
        ...
        console.error('❌ Error fetching laboratory support tickets:', error)
        setTickets([])
      } finally {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        console.log('📜 Fetching laboratory support history...')
        const response = await getSupportHistory()
        console.log('📜 Laboratory support history response:', response)
        
        if (response.success && response.data) {
          // Backend returns array directly in data
          const historyData = Array.isArray(response.data) ? response.data : []
          console.log('✅ Processed laboratory history:', historyData)
        ...
        console.error('❌ Error fetching laboratory support history:', error)
        setHistory([])
      } finally {
    ...
    <div className="mx-auto max-w-2xl lg:max-w-md py-6 lg:py-1">
      ...
      {activeTab === 'new' && (
        ...
        <form onSubmit={handleSubmit} className="space-y-5 lg:space-y-3">
          ...
          <div>
            <label htmlFor="labName" className="mb-1.5 lg:mb-1 block text-sm lg:text-xs font-semibold text-slate-700">
