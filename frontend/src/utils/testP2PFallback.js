...
const testP2PFallback = {
  ...
  printResults() {
    ...
    console.log(`\nFallback Detected: ${this.fallbackDetected ? '✅ YES' : '❌ NO'}`);
    ...
    if (this.fallbackDetected) {
      console.log('✅ SUCCESS: SFU fallback mechanism is working!');
    } else if (this.p2pEvents.length > 0 && this.sfuEvents.length === 0) {
      console.log('ℹ️  INFO: P2P is working, no fallback needed');
    } else {
      console.log('⚠️  WARNING: No events detected. Make sure a call is active.');
    }
  },
   ...
   */
  async runAllTests() {
    console.log('🧪 Starting P2P Fallback Test Suite...\n');
...
if (typeof window !== 'undefined') {
  ...
  setTimeout(() => {
    ...
    if (socket && socket.connected) {
      console.log('🔍 P2P Fallback Test Utility loaded. Run testP2PFallback.monitorFallback() to start monitoring.');
