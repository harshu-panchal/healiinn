...
exports.createAppointment = asyncHandler(async (req, res) => {
  ...
  try {
    console.log(
      `📅 Attempting to get/create session for doctor ${doctorId} on date ${appointmentDate}`
    );
...
exports.rescheduleAppointment = asyncHandler(async (req, res) => {
  ...
  while (tokenExists && attempts < maxAttempts) {
    ...
    if (cancelledTokenNumbers.has(newTokenNumber)) {
      console.log(
        `⚠️ Token ${newTokenNumber} belongs to a cancelled appointment (reschedule), skipping to ${
          newTokenNumber + 1
        }`
      );
      newTokenNumber++;
...
exports.verifyAppointmentPayment = asyncHandler(async (req, res) => {
  ...
  while (tokenExists && attempts < maxAttempts) {
    ...
    if (cancelledTokenNumbers.has(newTokenNumber)) {
      console.log(
        `⚠️ Token ${newTokenNumber} belongs to a cancelled appointment, skipping to ${
          newTokenNumber + 1
        }`
      );
      newTokenNumber++;
  ...
  console.log(
    `💳 Processing wallet credit for appointment: ${appointment._id}, doctorId: ${appointment.doctorId}`
  );
  if (!doctor) {
    console.error(
    ...
    console.log(
      `✅ Doctor found: ${doctor.firstName} ${
        doctor.lastName
      }, ID: ${doctor._id.toString()}`
    );
    console.log(
      `✅ Doctor found: ${doctor.firstName} ${doctor.lastName}, ID: ${doctor._id}`
    );
  ...
  console.log(
    `✅ Payment verification completed for appointment: ${appointment._id}`
  );
