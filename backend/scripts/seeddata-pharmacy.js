...
const createOrSkip = async (Model, data, identifier = 'email') => {
  try {
    ...
    if (existing) {
      console.log(`⏭️  ${Model.modelName} with ${identifier} "${data[identifier]}" already exists, skipping...`);
      return existing;
    }
    const created = await Model.create(data);
    console.log(`✅ Created ${Model.modelName}: ${data[identifier] || data.pharmacyName || data.name}`);
    return created;
  } catch (error) {
    if (error.code === 11000) {
      console.log(`⏭️  ${Model.modelName} with ${identifier} "${data[identifier]}" already exists (duplicate key), skipping...`);
...
const seedPharmacyData = async () => {
  try {
    console.log('🚀 Starting Pharmacy Seed Data...\n');
    ...
    for (let i = 0; i < medicineData.length; i++) {
      ...
      console.log(`✅ Created Medicine: ${medData.name} for ${pharmacy.pharmacyName}`);
    }
    
    console.log('\n📝 Creating Pharmacy Services...\n');
    ...
    console.log(`✅ Created ${pharmacyServices.length} Pharmacy Services`);
    
    console.log('\n📝 Creating Patients (if not exist)...\n');
    const patients = await getOrCreatePatients(8);
    
    console.log('\n📝 Getting or Creating Doctors and Prescriptions...\n');
    ...
    console.log('\n📝 Creating Pharmacy Notifications...\n');
    const notifications = [];
    ...
    console.log(`✅ Created ${notifications.length} Pharmacy Notifications`);
    ...
    console.log('\n✅ Pharmacy Seed Data Completed Successfully!\n');
    console.log('📊 Summary:');
