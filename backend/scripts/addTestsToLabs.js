...
const addTestsToLabs = async () => {
  try {
    console.log('🚀 Starting to Add Tests to Laboratories...\n');
    ...
    console.log(`🔍 Searching for laboratories with phone: ${phoneNumber}...\n`);
    ...
    if (laboratories.length === 0) {
      console.log(`❌ No laboratories found with phone number: ${phoneNumber}`);
      console.log('💡 Trying to find all approved laboratories...\n');
      ...
      if (allLabs.length === 0) {
        console.log('❌ No approved laboratories found in database.');
        process.exit(1);
      }
      
      console.log(`✅ Found ${allLabs.length} approved laboratories. Adding tests to all of them...\n`);
      ...
      console.log(`✅ Found ${laboratories.length} laboratory/laboratories with phone ${phoneNumber}:\n`);
...
const addTestsToLab = async (lab) => {
  try {
    console.log(`\n📝 Adding tests to: ${lab.labName} (${lab.phone})...\n`);
    ...
    for (const testInfo of testData) {
      ...
      if (existingTest) {
        console.log(`⏭️  Test "${testInfo.name}" already exists, skipping...`);
        skippedCount++;
        continue;
      }
      ...
      console.log(`✅ Created Test: ${testInfo.name} (₹${testInfo.price})`);
      addedCount++;
    }
    
    console.log(`\n📊 Summary for ${lab.labName}:`);
