...
const createOrSkip = async (Model, data, identifier = 'name') => {
  try {
    ...
    console.log(`✅ Created ${Model.modelName}: ${data[identifier] || data.email || data.labName || data.pharmacyName}`);
    return created;
  } catch (error) {
    if (error.code === 11000) {
...
const seedAdminData = async () => {
  try {
    ...
    if (!adminSettings) {
      ...
      console.log('⏭️  AdminSettings already exists');
    }
    
    console.log('\n📝 Creating Specialty Records...\n');
    ...
    console.log('\n✅ Admin Seed Data Completed Successfully!\n');
    console.log('📊 Summary:');
