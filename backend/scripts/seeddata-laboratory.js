...
const createOrSkip = async (Model, data, identifier = 'email') => {
  try {
    ...
    console.log(`✅ Created ${Model.modelName}: ${data[identifier] || data.labName || data.name}`);
    return created;
  } catch (error) {
    if (error.code === 11000) {
...
const seedLaboratoryData = async () => {
  try {
    console.log('🚀 Starting Laboratory Seed Data...\n');
    ...
    for (let i = 0; i < testData.length; i++) {
      ...
      tests.push(test);
      console.log(`✅ Created Test: ${testInfo.name} for ${lab.labName}`);
    }
    ...
    console.log('\n📝 Creating Laboratory Notifications...\n');
    const notifications = [];
    ...
    console.log(`✅ Created ${notifications.length} Laboratory Notifications`);
    ...
    console.log('\n✅ Laboratory Seed Data Completed Successfully!\n');
    console.log('📊 Summary:');
